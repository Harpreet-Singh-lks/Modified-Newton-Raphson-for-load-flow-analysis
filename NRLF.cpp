...
int main()
{
    ...
    int i, j, l, from_bus, to_bus, k, no_iterations, no_buses, no_lines, temp, precision_no = 8, no_PQ, no_pv, no_DG, linedata[120][3], no_tie;
    ...
    bool is_pu_data = true; // Set to true if data is in per-unit, false if in ohms

    // Optionally, read this flag from the input file or set it manually
    // Example: is_pu_data = true; // Uncomment if your data is in pu
    ...
    if (fp.is_open())
    {
        ...
        for (i = 1; i <= no_lines; i++)
        {
            ...
            // Convert to per-unit if data is in ohms
            if (!is_pu_data) {
                r = r / Z_base.real(); // Z_base is complex, use real part for ohmic base
                x = x / Z_base.real();
            }

           // z = complex<double>(r, (w0 / 1000.0) * x);
           z = complex<double>(r, x);  // direct use

            line_z[i] = z;

            if (abs(z) < 1e-10) {
                cout << "Warning: Zero impedance detected for line " << i << endl;
                continue;
            }
        ...
        for (i = 1; i <= no_buses; i++) {
            ...
            // Check if load type is provided
            if (fp >> temp) { // If load type is available
                if (temp == 1) { // Commercial load
                    LD[j].alpha = 1.51;
                    LD[j].beta = 3.4;
                } else if (temp == 2) { // Industrial load
                    LD[j].alpha = 0.18;
                    LD[j].beta = 6.0;
                } else if (temp == 3) { // Residential load
                    LD[j].alpha = 0.92;
                    LD[j].beta = 4.04;
                } else {
                    // Handle invalid load type
                    cout << "Invalid load type for bus " << j << ". Using default values." << endl;
                    LD[j].alpha = 1.2;
                    LD[j].beta = 2;
                }
            } else {
                // If load type is missing, assign default values
                cout << "Load type missing for bus " << j << ". Using default values." << endl;
                LD[j].alpha = 1.2;
                LD[j].beta = 2;
            }

            // Assign base load values
    ...
    V[1] = complex<double>(1.0, 0.0);
    for (i = 2; i <= no_buses; i++)
        V[i] = complex<double>(1.0, 0.0);
    ...
    for (k = 1; k <= no_iterations; k++)
    {
        ...
        for (i = 1; i <= no_lines; i++)
        {
            ...
            // Add these checks where division operations occur
            // For example, in the Y-bus formation:
            if (abs(z) < 1e-10) {  // Small threshold to avoid division by zero
                cout << "Warning: Zero impedance detected for line " << i << endl;
                continue;
            }
            Y[from_bus][to_bus] = Y[from_bus][to_bus] - Z_base / z;
        ...
        Sload = std::complex<double>(0.0, 0.0);
        for (i = 1; i <= no_buses; i++)
        {
        ...
        for (i = 2; i <= no_buses; i++)
        {
            ...
            complex<double> Yii = complex<double>(0.0, 0.0);
            int z;
        ...
        i = 1;
        {
            complex<double> temp = std::complex<double>(0.0, 0.0);
            for (l = 1; l <= no_buses; l++)
            {
                if (abs(Y[i][l]))
                {
        ...
        cout << "Jacobian Matrix Size: " << 2 * no_buses << " x " << 2 * no_buses << endl;
        cout << "Jacobian Matrix (Iteration " << k << "):" << endl;
        for (int row = 1; row <= 2 * no_buses; row++)
        {
            for (int col = 1; col <= 2 * no_buses; col++)
            {
                cout << std::setw(12) << J[row][col] << " ";
            }
            cout << endl;
        }
        ...
        // Add check for Jacobian matrix conditioning
        for (i = 1; i <= 2 * no_buses; i++) {
            for (j = 1; j <= 2 * no_buses; j++) {
                if (abs(J[i][j]) < 1e-10) {
                    J[i][j] = 0.0;  // Clean up very small values
                }
                if (isnan(J[i][j])) {
                    cout << "NaN in Jacobian at position [" << i << "][" << j << "]" << endl;
                    return 1;
                }
            }
        }
        ...
        // Debugging: Print updated voltages for all buses
        cout << "Iteration " << k << ": Updated Voltages\n";
        for (int bus = 1; bus <= no_buses; bus++) {
            cout << "Bus " << bus << ": Magnitude = " << abs(V[bus]) 
                 << ", Angle = " << arg(V[bus]) * 180 / PI << " degrees\n";
        }
        ...
        for (i = 1; i <= no_buses; i++) {
            if (isnan(abs(V[i])) || isnan(arg(V[i]))) {
                cout << "NaN detected at bus " << i << " in iteration " << k << endl;
                cout << "V[" << i << "] = " << V[i] << endl;
                cout << "Y[" << i << "][" << i << "] = " << Y[i][i] << endl;
                return 1;
            }
        }

        // Convergence check
        double maxP = max(delP, no_buses - 1); // Maximum mismatch for active power
        double maxQ = max(delP + no_buses - 1, no_buses); // Maximum mismatch for reactive power
        double freqMismatch = abs(delP[2 * no_buses - 1]); // Frequency mismatch
        double voltMismatch = abs(delP[2 * no_buses]); // Voltage mismatch
        // Print the convergence vector for this iteration
        cout << "Iteration " << k << ": maxP = " << maxP
        << ", maxQ = " << maxQ
        << ", freqMismatch = " << freqMismatch
        << ", voltMismatch = " << voltMismatch << endl;
        //Print the entire mismatch vector
        cout << "Mismatch vector (delP): ";
        for (int i = 1; i <= 2 * no_buses; i++) {
            cout << delP[i] << " ";
        }
        cout << endl;
        if (max({maxP}) < tolerance)
            break;
    }
